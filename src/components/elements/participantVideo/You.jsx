...
export default function Others(props) {
    return <div id={styles['you']} className="flex w-full h-full justify-end bg-contain bg-no-repeat bg-center bg-gray-800 p-4" style={{ backgroundImage: 'url(' + props.bg + ')' }}>
        <div className='flex gap-2'>
            ...
            <button className='flex items-center justify-center p-2 w-10 h-10 rounded-full'>{props.name} <MicIcon /></button>
            <button className='flex items-center justify-center p-2 w-10 h-10 rounded-full'>{props.name} <DotIcon /></button>
        </div>
